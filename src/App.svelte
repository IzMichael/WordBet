...
<script>
    ...
    let persistent = JSON.parse(window.localStorage.getItem('persistent')) || {};
    ...
    async function submit() {
        ...
        if (all.includes(input.toLowerCase())) {
            ...
            if (input == gamedata.word) {
                ...
                console.log(persistent.all[persistent.all.findIndex(w => w.word == gamedata.word)])
...
{:else if page == 'list'}
    <div class="flex flex-col items-center justify-start w-full h-full max-h-screen px-5 py-5 mx-auto font-mono lg:w-1/5 lg:px-0">
